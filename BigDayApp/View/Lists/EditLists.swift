...
protocol EditListProtocol {
    func didTapChoiceIcon()
    func didTapSaveList()
}

class EditLists: UIView {
    
    var delegate: EditListProtocol?
    
    private lazy var tittleList: UILabel = {
        let label = UILabel()
        label.text = "Qual o novo título da lista?"
    ...
    @objc func didTapSaveListButton() {
        delegate?.didTapSaveList()
    }
...
extension EditLists: SetupLayout {
    func addSubViews() {
